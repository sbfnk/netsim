/*! \file DimInfoVaccSIRS.hh
  \brief The Models::DimInfoVaccSIRS class.
*/
#ifndef DIMINFOVACCSIRS_HH
#define DIMINFOVACCSIRS_HH
...
namespace Models {
  ...
  */
  ...
  private:
    ...
    unsigned int mu; //!< Rate of vaccination.
...
*/
template <class Graph>
Models::DimInfoVaccSIRS<Graph>::DimInfoVaccSIRS(unsigned int v)
  : EpiModel<DimInfoState, Graph>("DimInfoVaccSIRS", v)
{
  ...
  this->rates.insert(std::make_pair("mu", &mu));
...
//----------------------------------------------------------
template <class Graph>
void Models::DimInfoVaccSIRS<Graph>::Init(const po::variables_map& vm,
  ...
  if (vm.count("effective")) {
    rec.push_back
      (new StatRecorder<Graph>
       (new write_effective_data<Graph, DimInfoVaccSIRS>(*this), 
        vm["effective"].as<double>()));
  }
  if (vm.count("info-dist")) {
    rec.push_back(new StatRecorder<Graph>
                  (new write_detail_dist<Graph, DimInfoVaccSIRS>,
                   vm["info-dist"].as<double>()));
  }
  if (vm.count("info-dis-corr")) {
    rec.push_back(new StatRecorder<Graph>
                  (new write_info_dis_corr<Graph, DimInfoVaccSIRS>(*this, 0),
                   vm["info-dis-corr"].as<double>()));
  }
  if (vm.count("risk-info")) {
    rec.push_back(new StatRecorder<Graph>
                  (new write_risk_info<Graph, DimInfoVaccSIRS>,
...
//----------------------------------------------------------
template <class Graph>
unsigned int Models::DimInfoVaccSIRS<Graph>::getNodeEvents(eventList& events,
                                                State* currentState,
                                                unsigned int nb) const
{
...
//----------------------------------------------------------
template <class Graph>
unsigned int Models::DimInfoVaccSIRS<Graph>::getEdgeEvents(eventList& events,
                                                State* currentState,
                                                unsigned int edge,
